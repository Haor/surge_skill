...
| `official/` | [services.nssurge.com](https://services.nssurge.com) | ZH | 1 | 官方服务接口说明（配置校验 API） |

**共计 122 个官方文件，约 14,700 行。**
...
`official/config-validate-api.md` — Surge 官方配置校验 API（Beta）：

- Endpoint: `POST https://services.nssurge.com/v1/config/validate`
- 支持直接 POST 配置文本，或 JSON `profile` 字段提交
- 返回 JSON：`valid`（是否合法）与 `error`（错误信息，实测为含 `message` 的对象）
